% ignores voxel identity, and simply computes mean an std of a reference
% batch and normalizes everyone else to it.

classdef refBatchTransformer < fmriDataTransformer
    ...
    methods
        function obj = zscoreBatchTransformer(batch_id_funhan, combat_opts)
        ...
        function cb_dat = transform(obj, dat)
            ...
            assert(size(dat.dat,1) == size(obj.ref_batch_img.dat,1), 'dat doesn''t match training dat dimensions');
            ...
            for i = 1:length(uniq_batch_id)
                this_idx = find(batch_id == uniq_batch_id(i));
                this_dat = cb_dat.dat(this_idx,:);
                this_std = std(this_dat(:));
                this_mu = mean(this_dat(:));
                zdat = (this_dat-this_mu)/this_std;
                cb_dat.dat(this_idx,:) = zdat * obj.ref_batch_std + obj.ref_batch_mu;
            end
        end
    end
    ...
    methods (Access = private)
        function obj = pick_ref_batch(obj, dat, batch_id)
            ...
            ref_batch = dat.get_wh_image(ref_batch_idx);
            
            obj.ref_batch_std = std(ref_batch.dat(:));
            obj.ref_batch_mu = mean(ref_batch.dat(:));
            obj.ref_batch_img = ref_batch.get_wh_image(1);
        end
    end
end